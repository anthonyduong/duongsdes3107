...
PFont font;
String letters = "I saw Cady Heron wearing army pants and flip-flops, so I bought army pants and flip flops";
int fontSizeMin = 4;
...
void setup() {
  ...
  image  = loadImage ("imageone.png");
  smooth();
  cursor(CROSS);
  x = mouseX;
  y = mouseY;
  font = createFont("Braggadocio-80.vlw", 5);
...
void draw() {
  ...
  if (mousePressed) {
    float d = dist(x, y, mouseY, mouseX);
    textFont(font, fontSizeMin+d/7);
